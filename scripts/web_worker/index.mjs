...
function settleBridgeTask(task, payload) {
  ...
  task.settled = true;
  clearInterval(task.timeoutHandle);
...
function touchBridgeTask(task) {
  if (!task || task.settled) {
    return;
  }
  task.lastActivityAtMs = Date.now();
}

function touchBridgeProviderTasks(provider) {
  for (const task of state.bridge.tasks.values()) {
    if (task.provider === provider && !task.settled) {
      touchBridgeTask(task);
    }
  }
}
...
function enqueueBridgeTask(provider, prompt, timeoutMs) {
  ...
  task.timeoutHandle = setInterval(() => {
    if (task.settled) {
      return;
    }
    if (Date.now() - task.lastActivityAtMs < timeoutMs) {
      return;
    }
    task.status = 'timeout';
    bridgeProgress(provider, 'bridge_timeout', '웹 연결 진행 신호가 멈췄습니다.');
    failBridgeTask(task, 'BRIDGE_TIMEOUT', `웹 연결 진행 신호 중단 (${timeoutMs}ms inactivity)`);
  }, 500);
...
async function handleBridgeHttpRequest(req, res) {
  ...
  if (pathname === '/v1/bridge/event' && req.method === 'POST') {
    ...
    const pageUrl = sanitizeUrlForUi(String(body.pageUrl ?? '').trim()) ?? null;
    recordBridgeSeen(provider, pageUrl);
    touchBridgeProviderTasks(provider);
...
async function extractFallbackProviderResponseText(provider, page) {
  const automation = providerAutomationConfig(provider);
  const candidates = await page.evaluate((selectors) => {
    const rows = [];
    for (const selector of selectors) {
    ...
    rows.sort((a, b) => a.bottom - b.bottom || a.len - b.len);
...
async function inspectProviderResponseState(provider, page, prompt, lastText) {
  const automation = providerAutomationConfig(provider);
  const [busyVisible, responseVisible] = await Promise.all([
    hasVisibleSelector(page, automation.busySelectors ?? []).catch(() => false),
    hasVisibleSelector(page, [
      ...automation.responseSelectors,
      '[data-message-author-role="assistant"]',
      '[data-message-author-role="model"]',
      'main article',
  ...
  const filteredText = await extractProviderResponseText(provider, page, prompt).catch(() => '');
  const fallbackText = filteredText || await extractFallbackProviderResponseText(provider, page).catch(() => '');

  if (busyVisible) {
    return {
      type: 'continue',
      text: '',
      message: '응답 생성 신호가 계속 보입니다. 대기를 이어갑니다.',
    };
  }
  if (fallbackText) {
    return {
  ...
  if (responseVisible || String(lastText ?? '').trim()) {
    return {
      type: 'continue',
      text: '',
      message: '응답 블록이 보여 추가 확인을 이어갑니다.',
    };
  }
  return {
    type: 'stalled',
    text: '',
    message: '응답 진행 신호와 결과 본문을 더 이상 확인할 수 없습니다.',
  };
}
...
async function waitForProviderResponse(provider, page, prompt, timeoutMs, runToken) {
  ...
  while (true) {
    ...
    if (decision.type === 'idle_timeout') {
      ...
      const probe = await inspectProviderResponseState(provider, page, prompt, lastText);
      if (probe.type === 'return_text' && probe.text) {
        return probe.text;
      }
      ...
      throw workerError('TIMEOUT', `${probe.message} (${idleTimeoutMs}ms inactivity)`);
    }
